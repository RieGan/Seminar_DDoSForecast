...
class LSTM_cell(object):
    ...
    def __init__(self, input_nodes, hidden_unit, output_nodes):
        ...
        self.Wo = tf.Variable(tf.random.truncated_normal([self.hidden_unit, self.output_nodes], mean=0, stddev=.01))
        self.bo = tf.Variable(tf.random.truncated_normal([self.output_nodes], mean=0, stddev=.01))

        # Placeholder for input vector with shape[batch, seq, embeddings]
        self._inputs = tf.keras.Input(dtype=tf.dtypes.float32, shape=[None, None, self.input_nodes], name='inputs')
