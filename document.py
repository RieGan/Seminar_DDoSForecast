...
def readEventList():
    events = []
    id = 0
    for line in open('data/target-event.u8.txt', 'r').readlines():
        line = line.strip()
        if len(line) > 0:
            p = line.split('\t')
            if len(p) == 2:
...
def readRandomDate():
    dates = []
    for line in open('data/random-date.txt', 'r').readlines():
        line = line.strip()
        if len(line) > 0:
...
def readIDList():
    idList = []
    for line in open('data/id-list.txt', 'r').readlines():
...
def initData_multi():
    neg = readTweets('data/negative.txt')
    pos = readTweets('data/positive.txt')
    events = readEventList()
